...
def warning_stock() -> list:
    sql = text(
        f'select id, `date`, stock_code, stock_name, data, warning from warning where `date`={datetime.today().strftime("%Y%m%d")}')
    result = []
    with engin.connect() as connection:
        res = connection.execute(sql)
        connection.commit()
        for row in res:
            id, _date, code, name, df, warning = row
            csv_data = io.StringIO(df)
            df = pd.read_csv(csv_data)
            # print(row)
            df.replace([float('inf'), float('-inf')], float('nan'), inplace=True)
